...
def  grid_multiple(window):
	""" takes the largest product in the same direction within some window """
	arr = []

	arr.append([8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 5, 77, 91, 8])
	arr.append([49, 49, 99, 4, 17, 81, 18, 57, 60, 87, 17, 40, 98, 43, 69, 48, 4, 56, 62, 0])
	...
	arr.append([16, 39, 5, 42, 96, 35, 31, 47, 55, 58, 88, 24, 0, 17, 54, 24, 36, 29, 85, 57])
	...
	for i in range(0, len(arr)):
		...
		# top-left-to-bottom-right diagonal sum
			if (j < len(row) - window) and (i < len(arr) - window):
				product = 1
				k = 0
				while k < window:
					product *= arr[i + k][j + k]
		...
		# top-right-to-bottom-left diagonal sum
			if (j > window - 2) and (i < len(arr) - window):
				product = 1
				k = 0
				while k < window:
					product *= arr[i + k][j - k]
...
print(grid_multiple(4)) # 70600674
